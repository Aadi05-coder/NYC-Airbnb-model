...
from utils import get_enriched_df, BOROUGH_COLORS, ROOM_COLORS, PLOT_LAYOUT, inject_styles
import copy
...
def render():
    ...
    st.markdown('<p class="section-header" style="margin-top:0.5rem">Price Distribution by Borough</p>',
                unsafe_allow_html=True)
    fig_box = go.Figure()
    ...
    st.markdown('<p class="section-header" style="margin-top:0.5rem">Listing Density Map</p>',
                unsafe_allow_html=True)
    st.markdown('<p class="section-sub">Geographic spread of 48 k listings across the five boroughs</p>',
                unsafe_allow_html=True)
