...
import copy
from utils import get_enriched_df, get_encoded_df, BOROUGH_COLORS, ROOM_COLORS, PLOT_LAYOUT, inject_styles
...
def render():
    ...
    <div style="padding:2rem 0 1rem">
        <p style="color:#5050a0;font-size:0.8rem;letter-spacing:0.12em;
    ...
    with tab1:
        ...
        with c1:
            ...
            fig_hist = go.Figure()
            fig_hist.add_trace(go.Histogram(x=df["price_orig"], nbinsx=60,
                                            marker_color="#6060c8", opacity=0.8))
        ...
        with c2:
            ...
            fig_av = go.Figure()
            fig_av.add_trace(go.Histogram(x=df["availability_365"], nbinsx=50,
                                          marker_color="#ff7bbd", opacity=0.8))
        ...
        col_a.markdown("""
        <div class="insight-card"><p>Before outlier removal, prices above <strong>$334</strong>
        were treated as outliers using the IQR method (Q3 + 1.5×IQR).</p></div>
        <div class="insight-card"><p>IQR bounds: <strong>lower = $0</strong>,
        <strong>upper = $334</strong> — anything beyond was dropped.</p></div>
        """, unsafe_allow_html=True)
        col_b.markdown("""
        <div class="insight-card"><p>After cleaning, <strong>48,360 listings</strong> remain
        — roughly <strong>5% removed</strong>.</p></div>
        <div class="insight-card"><p>Resulting range: <strong>$10–$334</strong>
        · Median ≈ <strong>$92</strong> · Mean ≈ <strong>$106</strong>.</p></div>
        """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        fig_bgroup.add_trace(go.Bar(
            name="Avg Price", x=borough_stats["neighbourhood_group"],
            y=borough_stats["Avg Price"], marker_color="#6060c8",
            text=borough_stats["Avg Price"].apply(lambda v: f"${v:.0f}"),
            textposition="outside",
        ))
        fig_bgroup.add_trace(go.Bar(
            name="Median Price", x=borough_stats["neighbourhood_group"],
            y=borough_stats["Median Price"], marker_color="#ff7bbd",
        ...
        st.markdown('<p class="section-header" style="margin-top:1rem">Top 15 Neighbourhoods by Avg Price</p>',
                    unsafe_allow_html=True)
        top_n = (
    ...
    with tab3:
        ...
        with c1:
            ...
            rt_avg = (df.groupby("room_type")["price_orig"].mean()
                      .reset_index().sort_values("price_orig", ascending=False))
    ...
    with tab4:
        ...
        for title, body in [
            ("Room type → Price",
             "Room type is the strongest single predictor. Entire homes command 2–3× the rate of shared rooms."),
            ("Reviews ↔ Availability",
             "Listings with more reviews tend to have higher availability — active, well-managed properties."),
            ("Minimum nights",
             "Longer minimum-night requirements correlate weakly with lower review counts."),
            ("Host listings count",
             "Hosts with many listings price competitively — slight negative correlation with per-listing price."),
        ]:
            st.markdown(
                f'<div class="insight-card"><p><strong>{title}</strong> — {body}</p></div>',
                unsafe_allow_html=True,
            )
