...
from utils import get_enriched_df, PLOT_LAYOUT, inject_styles
...
def render():
    ...
    <div style="padding:2rem 0 1rem">
        <p style="color:#5050a0;font-size:0.8rem;letter-spacing:0.12em;
                  text-transform:uppercase;margin-bottom:0.3rem">MODEL EVALUATION</p>
    ...
    head[0].markdown("<p style='color:#4040a0;font-size:0.78rem;letter-spacing:0.08em'>METRIC</p>",
                     unsafe_allow_html=True)
    head[1].markdown("<p style='color:#7b7bff;font-size:0.78rem;letter-spacing:0.08em'>LINEAR REG</p>",
                     unsafe_allow_html=True)
    head[2].markdown("<p style='color:#ffd07b;font-size:0.78rem;letter-spacing:0.08em'>RANDOM FOREST</p>",
                     unsafe_allow_html=True)
    head[3].markdown("<p style='color:#7bffcc;font-size:0.78rem;letter-spacing:0.08em'>XGBOOST</p>",
                     unsafe_allow_html=True)
    ...
    for label, lrv, rfv, xgbv in [
        ...
        rc[0].markdown(f"<p style='color:#9090c0;padding:0.4rem 0'>{label}</p>",
                       unsafe_allow_html=True)
        rc[1].markdown(f"<p style='color:#c0c0ff;font-weight:600;padding:0.4rem 0'>{lrv}</p>",
                       unsafe_allow_html=True)
        rc[2].markdown(f"<p style='color:#ffd07b;font-weight:600;padding:0.4rem 0'>{rfv}</p>",
                       unsafe_allow_html=True)
        rc[3].markdown(f"<p style='color:#7bffcc;font-weight:600;padding:0.4rem 0'>{xgbv}</p>",
                       unsafe_allow_html=True)
    ...
    st.markdown('<p class="section-header" style="margin-top:0.5rem">Actual vs Predicted — XGBoost</p>',
                unsafe_allow_html=True)
    fig_s = go.Figure()
    ...
    for k, v in [
        ("Features used",
         "room_type, neighbourhood_group, neighbourhood, minimum_nights, number_of_reviews, "
         "reviews_per_month, host_listings_count, availability_365"),
        ("Target variable",
         "Normalised price (z-score) → converted back to USD (mean=$106, std=$64)"),
        ("Outlier removal",
         "IQR method — prices above ~$334/night removed before modelling"),
        ("Train / Test split",
         "80% train · 20% test · random_state=42"),
        ("XGBoost config",
         "n_estimators=300 · learning_rate=0.05 · max_depth=8 · random_state=42"),
        ("Random Forest config",
         "n_estimators=200 · max_depth=10 · random_state=42 · n_jobs=-1"),
    ]:
        st.markdown(
            f'<div class="insight-card"><p><strong>{k}</strong> — {v}</p></div>',
            unsafe_allow_html=True,
        )
